...
**WebUI 本体（rc.2 壳 + 网关协议面）**：3088 `/` 伺服真 DSH GUI 壳（vendored dist + 42 插件 + 捕获的生成版 shell.html 含 `__DSH_BOOT__`；资产全量 base64 侧车——文本面 UTF-8 往返丢字节实锤）。网关协议：`/api/events.mux|host` **双传输**（浏览器 WS 下行 + Node SSE——rc.2 浏览器端实为 WebApiClient.readWebSocket）；`/api/<method>` unary POST（client-request 全形信封，17 方法：host/workspace/settings/llm/session CRUD+search+prompt/cancel/subagent）。写路径：`session.prompt` → 真 LLM 回合（turn/start → user/message → step/start → assistant/chunk 流式[reasoning+text] → assistant/message → step/end → turn/end）→ mux 实况广播。浏览器连接排障链（5 只真 bug）：SSE-only 致 WS 握手 200 即断、callGuest conn_id=0 致 push 永不中、MAX_ROUTES 16 溢出静默挤掉 /ws、**WS 不回 pong**（Firefox 主动 ping 即断——node ws 库宽容故全绿，Node/Firefox 差异实锤）、scale-* 探针会话噪音直灌 GUI（已过滤——GUI 视野=干净 chat 会话）。诊断面：`/debug/gateway`（unaryMiss/push 回执/浏览器错误探针回报）。
